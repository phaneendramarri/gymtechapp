...
export const PricingSection: React.FC = () => {
  ...
  return (
    <section id="pricing" className="py-28 sm:py-40 border-y border-line/70 bg-cream">
          ...
          {PLANS.map((p, idx) => (
            <motion.div
              ...
              className={`relative flex flex-col rounded-[1.75rem] border p-8 transition-all duration-300 hover:-translate-y-1 ${
                p.highlight
                  ? 'border-iron bg-bg shadow-[0_0_0_1px_var(--iron),0_12px_40px_rgba(251,146,60,0.12)]'
                  : 'border-line bg-surface hover:border-iron/40 hover:shadow-[0_12px_32px_-12px_rgba(0,0,0,0.15)]'
              }`}
            >
              {/* Highlighted badge */}
              {p.highlight && (
              ...
              <Button
                asChild
                className={`mt-6 h-11 gap-2 rounded-full font-semibold text-[14px] transition-all ${
                  p.highlight
                    ? 'bg-iron hover:bg-iron-hover text-white shadow-[0_4px_16px_rgba(251,146,60,0.25)] hover:shadow-[0_6px_24px_rgba(251,146,60,0.35)] hover:-translate-y-0.5 active:scale-[0.98]'
