...
// Social icon SVGs — consistent with the rest of the design system
...
const SocialLink: React.FC<{ href: string; label: string; children: React.ReactNode }> = ({ href, label, children }) => (
  <a
    ...
    rel="noreferrer"
    aria-label={label}
    className="size-8 rounded-lg border border-(--line)/60 flex items-center justify-center text-ink-3 hover:text-(--iron) hover:border-(--iron)/40 transition-all duration-200"
  >
    {children}
  </a>
);
...
export const FooterSection: React.FC = () => {
  return (
    <footer className="border-t border-(--line) bg-(--surface)/60 backdrop-blur-md py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 mb-12">
          ...
          <div className="space-y-3 md:col-span-2">
            ...
            <a
              href="/status"
              className="inline-flex items-center gap-2 text-[11px] font-mono text-(--iron) pt-1 font-semibold hover:underline"
            >
          ...
          {/* Col 2: Product */}
          <div>
            <h4 className="font-mono text-xs font-bold uppercase tracking-wider text-ink mb-3">Product</h4>
            <ul className="space-y-2 text-xs text-ink-3">
              <li><a href="#product" className="hover:text-(--iron) transition-colors">Product Workflow</a></li>
              <li><a href="#how-it-works" className="hover:text-(--iron) transition-colors">How It Works</a></li>
              <li><a href="#pricing" className="hover:text-(--iron) transition-colors">Pricing</a></li>
              <li><a href="#faq" className="hover:text-(--iron) transition-colors">FAQ</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs text-ink-3">
              ...
              <li><a href="mailto:hello@gymtech.in" className="hover:text-(--iron) transition-colors">hello@gymtech.in</a></li>
          ...
          {/* Col 4: Company */}
          <div>
            <h4 className="font-mono text-xs font-bold uppercase tracking-wider text-ink mb-3">Company</h4>
            <ul className="space-y-2 text-xs text-ink-3">
              <li><a href="/about" className="hover:text-(--iron) transition-colors">About</a></li>
              <li><a href="/careers" className="hover:text-(--iron) transition-colors">Careers</a></li>
              <li><a href="/press" className="hover:text-(--iron) transition-colors">Press</a></li>
              <li><a href="/contact" className="hover:text-(--iron) transition-colors">Contact</a></li>
            </ul>
          </div>
        </div>

        <div className="border-t border-(--line)/60 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] font-mono text-ink-3">
