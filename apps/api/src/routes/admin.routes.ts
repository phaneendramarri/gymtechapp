...
adminRoutes.patch('/licenses', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid license payload', 400);
  const licenseRepo = new LicenseRepository(ctx.env.DB, 0);
...
adminRoutes.post('/gyms', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid gym data', 400);
  const adminRepo = new AdminRepository(ctx.env.DB);
  try {
...
adminRoutes.post('/gyms/:id/status', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid status payload', 400);
  const adminRepo = new AdminRepository(ctx.env.DB);
...
adminRoutes.put('/gyms/:id/features', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid features payload', 400);
  const adminRepo = new AdminRepository(ctx.env.DB);
...
adminRoutes.put('/users/:id', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid user update payload', 400);
  const adminRepo = new AdminRepository(ctx.env.DB);
...
adminRoutes.put('/gyms/:id/license-limits', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid license limits payload', 400);
  const adminRepo = new AdminRepository(ctx.env.DB);
...
adminRoutes.put('/communications', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid gateway config', 400);
  await ctx.env.DB
...
adminRoutes.post('/communications/test-smtp', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid SMTP test payload', 400);
...
adminRoutes.post('/gyms/:id/top-up-credits', safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid top-up payload', 400);
  const licenseRepo = new LicenseRepository(ctx.env.DB, 0);
