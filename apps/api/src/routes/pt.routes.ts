...
ptRoutes.post('/collections', requireGym, requireFeature('pt_collections'), safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid PT collection payload', 400);
...
ptRoutes.post('/collections/:id/settle', requireGym, requireRole('OWNER', 'MANAGER'), safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr('Invalid settlement payload', 400);
