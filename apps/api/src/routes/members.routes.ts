...
memberRoutes.post('/', requireGym, safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid member data', 400);
...
memberRoutes.post('/bulk-import', requireGym, safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid bulk import payload', 400);
  const memberRepo = new MemberRepository(ctx.env.DB, ctx.gymId!);
...
memberRoutes.put('/:id', requireGym, safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid update payload', 400);
...
memberRoutes.post('/:id/renew', requireGym, requireFeature('members'), safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid renewal payload', 400);
  const memberRepo = new MemberRepository(ctx.env.DB, ctx.gymId!);
...
memberRoutes.post('/:id/freeze', requireGym, requireRole('OWNER', 'MANAGER'), safeHandler(async (c) => {
  ...
  if (!parsed.success) return jsonErr(parsed.error.errors[0]?.message || 'Invalid freeze payload', 400);
